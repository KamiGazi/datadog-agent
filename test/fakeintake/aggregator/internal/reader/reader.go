...
func (r *MetricDataReader) Initialize() error {
	...
	r.dictUnitStr, r.unitRefs, err = unpackUnitFields(r.data.ProtoReflect().GetUnknown())
	if err != nil {
		return err
	}
	return nil
...
const (
	...
	fieldDictUnitStr protowire.Number = 25
	fieldUnitRefs    protowire.Number = 26
)
...
func unpackUnitFields(raw []byte) ([]string, []int64, error) {
	dictUnitStr := []string{""}
	var unitRefs []int64
	...
	for len(raw) > 0 {
		...
		case fieldDictUnitStr:
			...
			dict, err := unpackStrDict(value)
			if err != nil {
				return nil, nil, err
			}
			dictUnitStr = append(dictUnitStr, dict[1:]...)
			raw = raw[n:]
		case fieldUnitRefs:
			if typ != protowire.BytesType {
				return nil, nil, errBadReference
			...
			refs, err := unpackPackedSint64(value)
			if err != nil {
				return nil, nil, err
			}
			unitRefs = append(unitRefs, refs...)
			raw = raw[n:]
		default:
	...
	return dictUnitStr, unitRefs, nil
}

func unpackPackedSint64(raw []byte) ([]int64, error) {
	var values []int64
	for len(raw) > 0 {
		...
		values = append(values, int64(value>>1)^-int64(value&1))
		raw = raw[n:]
	}
	return values, nil
}
...
func (r *MetricDataReader) NextMetric() error {
	...
	r.currentUnitRef = 0
	if r.HasUnit() {
		if r.unitRefsIdx >= len(r.unitRefs) {
			return errUnexpectedEOF
		}
		r.unitRef += r.unitRefs[r.unitRefsIdx]
		r.unitRefsIdx++
		if r.unitRef < 0 || r.unitRef >= int64(len(r.dictUnitStr)) {
			return errBadReference
		}
		r.currentUnitRef = r.unitRef
	}
...
// HasUnit returns whether the current metric entry carries a unit.
func (r *MetricDataReader) HasUnit() bool {
	return r.packedType()&flagHasUnit != 0
}

// Unit returns unit for the current metric entry.
func (r *MetricDataReader) Unit() string {
	return r.dictUnitStr[r.currentUnitRef]
}
