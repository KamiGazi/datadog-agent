...
// Package invalidsysprobeconfig reports system-probe schema violations through the Agent Health Platform.
...
// checker validates the system-probe in-memory config against the embedded
// system-probe schema
type checker struct {
...
func (c *checker) validate() ([]storedef.IssueReport, error) {
	if c.cfg == nil {
		// subcommand didn't bundle sysprobeconfig so there is nothing to validate
		return nil, nil
	}
