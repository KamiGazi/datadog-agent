...
func (c *converterWithoutAgent) ensureOtlpHTTPExporterConfig(conf confMap, exporterNames []any) error {
	...
	for _, nameAny := range exporterNames {
		if name, ok := nameAny.(string); ok && confmaputils.IsComponentType(name, componentTypeOtlpHTTP) {
			hasOtlpHTTP = true
...
func (c *converterWithoutAgent) removeAgentOnlyExtensions(conf confMap) error {
	...
	for _, extAny := range extensions {
		...
		if confmaputils.IsComponentType(ext, componentTypeDDProfiling) || confmaputils.IsComponentType(ext, componentTypeHPFlare) {
			continue
		}
	...
	if ok {
		for name := range extensionsConf {
			if confmaputils.IsComponentType(name, componentTypeDDProfiling) || confmaputils.IsComponentType(name, componentTypeHPFlare) {
...
func (c *converterWithoutAgent) addInternalHealthMetricsPipeline(conf confMap, profilesExporterNames []any, profilesProcessors []any) error {
	...
	for _, exporterNameAny := range profilesExporterNames {
		...
		if !confmaputils.IsComponentType(exporterName, componentTypeOtlpHTTP) {
			continue
		}
		...
		if !ok {
			slog.Warn("otlp_http exporter missing profiles_endpoint, cannot infer metrics endpoint",
