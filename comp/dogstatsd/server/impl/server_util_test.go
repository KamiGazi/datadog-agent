...
// fulfillDepsWithInactiveServerlessServer is the serverless-mode counterpart
// of fulfillDepsWithInactiveServer: it returns a *dsdServer that has not been
// started and has ServerlessMode=true, matching the production serverless-init
// path where the server is constructed but never started (e.g. missing api_key).
func fulfillDepsWithInactiveServerlessServer(t *testing.T, cfg map[string]interface{}) (depsWithoutServer, *dsdServer) {
	return fulfillDepsWithInactiveServerAndParams(t, cfg, true)
}
