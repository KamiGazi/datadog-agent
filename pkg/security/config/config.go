...
func NewRuntimeSecurityConfig() (*RuntimeSecurityConfig, error) {
	...
	rsConfig := &RuntimeSecurityConfig{
		...
		SysCtlSnapshotKernelCompilationFlags: map[string]uint8{},
		SyscallEventsEnabled:                 pkgconfigsetup.SystemProbe().GetBool("runtime_security_config.syscall_events.enabled"),
		SyscallEventsPeriod:                  pkgconfigsetup.SystemProbe().GetDuration("runtime_security_config.syscall_events.period"),
